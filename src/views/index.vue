  ...
  <div v-if="data" class="index">
    ...
    <step-list>
      <step-list-item :order="1">
        ...
        <template v-if="data.gameEvent[0].stepSetting.status === '进行中' || data.gameEvent[0].stepSetting.status === '已完成'" #extend>
          <tutorial-list>
...
<!--            </tutorial-list-item>-->
      ...
      <template v-for="i in 5">
        <step-list-item v-if="data.gameEvent[i]" :key="i" :order="i + 1" :disabled="data.gameEvent[i].stepSetting.status === '未开始' && data.gameEvent[i - 1].stepSetting.status !== '已完成'">
          ...
          <template v-if="data.gameEvent[i].stepSetting.status === '进行中' || data.gameEvent[i].stepSetting.status === '已完成'" #extend>
            <task-list>
...
export default {
  ...
  methods: {
    ...
    openQuestion() {
      window.open('https://www.wenjuan.com/s/UZBZJvaVIH/# ', '_blank');
    },
    async getAllData() {
